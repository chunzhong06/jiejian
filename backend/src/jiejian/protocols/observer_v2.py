...
#   脱敏数据。V1 Observation 的兼容投影由执行边界显式完成。
# =============================================================================
...
class OwnerApiLocatorV2(ObserverModel):
...
class SqliteQueryLocatorV2(ObserverModel):
...
class QueuePeekBudgetV2(ObserverModel):
...
class AzureQueuePeekLocatorV2(ObserverModel):
...
class BlobObjectScanBudgetV2(ObserverModel):
...
class AzureBlobObjectLocatorV2(ObserverModel):
...
class AuditLogScanBudgetV2(ObserverModel):
...
class StructuredAuditLogLocatorV2(ObserverModel):
    ...
    scan_budget: AuditLogScanBudgetV2
...
class AsyncTaskPollBudgetV2(ObserverModel):
...
class AsyncTaskApiLocatorV2(ObserverModel):
    ...
    allow_loopback_http: bool
    poll_budget: AsyncTaskPollBudgetV2

    @model_validator(mode="after")
    def validate_locator(self) -> AsyncTaskApiLocatorV2:
...
class ObserverSpecV2(ObserverModel):
    ...
    required: bool
    budget: ObserverBudgetV2

    @model_validator(mode="after")
    def validate_spec(self) -> ObserverSpecV2:
        ...
        if self.observer_type is ObserverType.ASYNC_TASK_STATUS:
            ...
            locator = self.target.locator
            if not isinstance(locator, AsyncTaskApiLocatorV2) or locator.poll_budget.max_response_bytes > self.budget.max_bytes:
        ...
        if self.observer_type is ObserverType.AZURE_BLOB_OBJECT:
            if not isinstance(locator, AzureBlobObjectLocatorV2):
...
class CorrelationV2(ObserverModel):
...
class AuditLogStartCursorV2(ObserverModel):
    ...
    @model_validator(mode="after")
    def validate_anchor(self) -> AuditLogStartCursorV2:
...
class ObserverInvocationV2(ObserverModel):
    spec: ObserverSpecV2
    correlation: CorrelationV2
    phase: ObservationPhase

    @model_validator(mode="after")
    def validate_invocation(self) -> ObserverInvocationV2:
        if self.spec.observer_type is ObserverType.STRUCTURED_AUDIT_LOG:
            raise ValueError("structured audit observers require AuditLogObserverInvocationV2")
        if self.spec.observer_type is ObserverType.ASYNC_TASK_STATUS:
            raise ValueError("async task observers require AsyncTaskObserverInvocationV2")
        if self.phase not in self.spec.phases:
            raise ValueError("observer invocation phase is not declared by the spec")
        return self


class AsyncTaskObserverInvocationV2(ObserverModel):
    spec: ObserverSpecV2
    correlation: CorrelationV2
    phase: ObservationPhase

    @model_validator(mode="after")
    def validate_invocation(self) -> AsyncTaskObserverInvocationV2:
...
class AuditLogObserverInvocationV2(ObserverModel):
    spec: ObserverSpecV2
    correlation: CorrelationV2
    phase: ObservationPhase
    start_cursors: tuple[AuditLogStartCursorV2, ...] = ()

    @model_validator(mode="after")
    def validate_invocation(self) -> AuditLogObserverInvocationV2:
        ...
        if not isinstance(locator, StructuredAuditLogLocatorV2):
...
class ObservationWindowV2(ObserverModel):
    phase: ObservationPhase
    ...
    @model_validator(mode="after")
    def validate_window(self) -> ObservationWindowV2:
...
def build_normalized_state(payload: Mapping[str, Any], *, known_secrets: tuple[str, ...] = ()) -> NormalizedStateV2:
...
class ObservationProvenanceV2(ObserverModel):
    ...
    @model_validator(mode="after")
    def validate_provenance(self) -> ObservationProvenanceV2:
...
class ObservationEnvelopeV2(ObserverModel):
    observer_id: str = Field(pattern=_ID_PATTERN)
    observer_type: ObserverType
    protocol_version: Literal["2"] = "2"
    phase: ObservationPhase
    ...
    state: NormalizedStateV2 | None = None
    provenance: ObservationProvenanceV2 | None = None
    ...
    @model_validator(mode="after")
    def validate_envelope(self) -> ObservationEnvelopeV2:
...
class ObserverOutcomeV2(ObserverModel):
    observer_id: str = Field(pattern=_ID_PATTERN)
    required: bool
...
def parse_observer_json(
    ...
    *,
    ...
    # duplicate-key/nonfinite/BOM checks above are performed on the decoded tree;
    # Pydantic's JSON entrypoint then preserves strict model semantics while
    # decoding enum and tuple wire values.
