...
class FlowDraftVariableSourceV1(FlowDraftProtocolModel):
...
class FlowDraftVariableV1(FlowDraftProtocolModel):
    name: str = Field(pattern=PROJECT_ID_PATTERN)
    placeholder: str = Field(pattern=r"^\{[a-z][a-z0-9_-]{0,63}\}$")
    status: FlowDraftVariableStatus
    candidate_sources: tuple[FlowDraftVariableSourceV1, ...] = Field(
        min_length=1,
        max_length=128,
    )
    confirmed_source: FlowDraftVariableSourceV1 | None = None
    consumer_step_ids: tuple[str, ...] = Field(min_length=1, max_length=128)

    @model_validator(mode="after")
    def validate_source_state(self) -> FlowDraftVariableV1:
        candidates = {
...
class FlowDraftStepV1(FlowDraftProtocolModel):
    ...
    @model_validator(mode="after")
    def validate_step_boundary(self) -> FlowDraftStepV1:
...
class FlowDraftV1(FlowDraftProtocolModel):
    recording_id: str = Field(pattern=RECORDING_ID_PATTERN)
    flow_id: str = Field(pattern=PROJECT_ID_PATTERN)
    revision: int = Field(ge=1)
    steps: tuple[FlowDraftStepV1, ...] = Field(min_length=1, max_length=2_000)
    variables: tuple[FlowDraftVariableV1, ...] = Field(default=(), max_length=2_000)
    owner_observer_path: str = "/owner/resources/{resource_id}"
    reset_path: str = "/reset"

    @model_validator(mode="after")
    def validate_draft_graph(self) -> FlowDraftV1:
...
class DeleteFlowDraftStepV1(FlowDraftProtocolModel):
    operation: Literal["DELETE_STEP"]
    step_id: str = Field(pattern=PROJECT_ID_PATTERN)


class MergeFlowDraftStepsV1(FlowDraftProtocolModel):
...
class RenameFlowDraftStepV1(FlowDraftProtocolModel):
    operation: Literal["RENAME_STEP"]
    step_id: str = Field(pattern=PROJECT_ID_PATTERN)
    name: str = Field(min_length=1, max_length=128)


class ConfirmFlowDraftVariableV1(FlowDraftProtocolModel):
...
FlowDraftReviewCommandV1: TypeAlias = Annotated[
...
_COMMAND_ADAPTER = TypeAdapter(FlowDraftReviewCommandV1)
_COMMAND_TYPES = (
    DeleteFlowDraftStepV1,
    MergeFlowDraftStepsV1,
    RenameFlowDraftStepV1,
...
def canonical_flow_draft_json_bytes(
    document: FlowDraftV1 | FlowDraftReviewCommandV1,
    *,
    known_secrets: Sequence[str] = (),
) -> bytes:
    """生成草稿或审阅命令的规范 JSON，并拒绝已知秘密。"""

    if not isinstance(document, (FlowDraftV1, *_COMMAND_TYPES)):
        raise TypeError("flow draft canonical JSON requires a V1 document")
    ...
    maximum = (
        ...
        if isinstance(document, FlowDraftV1)
...
def parse_flow_draft(
    ...
    *,
    ...
    del parsed
    try:
        return FlowDraftV1.model_validate_json(raw, strict=True)
