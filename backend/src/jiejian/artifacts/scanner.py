...
from ..execution.published_artifacts import PublicationManifestV1
from .models import (
...
def scan_artifact(request: ArtifactCheckRequestV1, *, clock: Callable[[], float] = time.monotonic) -> ArtifactScanResultV1:
    ...
    try:
        ...
        findings: list[ArtifactFindingV1] = []
...
def _load_manifest(root: Path, manifest_path: Path) -> tuple[bytes, PublicationManifestV1]:
    ...
    try:
        manifest = PublicationManifestV1.model_validate_json(raw, strict=True)
    except Exception:
        raise ArtifactScanFailure("MANIFEST_INVALID") from None
    return raw, manifest


def _manifest_entries(root: Path, manifest: PublicationManifestV1) -> tuple[tuple[str, int, str], ...]:
...
def _scan_one_file(
    request: ArtifactCheckRequestV1,
    relative: str,
    raw: bytes,
    manifest_hash: str,
    findings: list[ArtifactFindingV1],
...
def _add_finding(
    ...
    evidence.append(ArtifactEvidenceV1(evidence_id=evidence_id, artifact_id=request.artifact_id, manifest_sha256=manifest_hash, rule_id=rule_id, path=path, fingerprint=fingerprint, line=line, reason_code=rule_id))
    findings.append(ArtifactFindingV1(finding_id=finding_id, artifact_id=request.artifact_id, rule_id=rule_id, category=category, severity=severity, path=path, evidence_id=evidence_id, message=message))


def _result(request: ArtifactCheckRequestV1, manifest_hash: str, file_count: int, total_bytes: int, findings: list[ArtifactFindingV1], evidence: list[ArtifactEvidenceV1]) -> ArtifactScanResultV1:
    return ArtifactScanResultV1(
        project_id=request.project_id,
        artifact_id=request.artifact_id,
        run_id=request.run_id,
...
def _check_time(started: float, request: ArtifactCheckRequestV1, clock: Callable[[], float]) -> None:
