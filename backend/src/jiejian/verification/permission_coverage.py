# =============================================================================
# Verification V2 关系覆盖规划
#
# 定位
#   将显式 V2 权限意图编译为有限、可解释、确定性的关系覆盖计划。
#
# 安全边界
...
class PermissionMutationCaseV2(PermissionModel):
    ...
    def validate_shape(self) -> PermissionMutationCaseV2:
...
class PermissionMutationPlanV2(PermissionModel):
    ...
    cases: tuple[PermissionMutationCaseV2, ...] = Field(max_length=8192)
    ...
    @model_validator(mode="after")
    def validate_counts(self) -> PermissionMutationPlanV2:
...
def _path_index(contract: PermissionContractV2) -> dict[tuple[str, str], tuple[RelationFact, ...]]:
...
def _missing_observers(required: tuple[str, ...], available: set[str]) -> bool:
...
def _nearest_workflow_resource(contract: PermissionContractV2, rule: PermissionRuleV2) -> str | None:
...
def _ordinary_candidates(
    ...
    if rule.resource_id not in available_resources:
        ...
        return []
    if _missing_observers(rule.required_observers, available_observers):
        for dimension, expectation in targets:
...
def build_permission_coverage_plan(
    ...
    *,
    ...
    available_observers: tuple[str, ...] = ("http", "owner_api"),
    ...
    for rule in contract.rules:
        raw.extend(_ordinary_candidates(contract, rule, available_subjects, available_resources, set(available_observers), max_relation_depth, gaps, engine_version))
    for rule in contract.batch_rules:
        raw.extend(_batch_candidates(contract, rule, available_subjects, available_resources, set(available_observers), gaps, engine_version, max_relation_depth))
    ...
    for rule in (*contract.rules, *contract.batch_rules):
        targets = ((CoverageDimension.BULK, None),) if isinstance(rule, BatchPermissionRuleV2) else tuple(
    ...
    return PermissionMutationPlanV2(
