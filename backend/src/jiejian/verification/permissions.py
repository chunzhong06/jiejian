...
#   本模块不依赖适配器，也不执行 Runner；V1 适配器仅保留历史所有权用例的语义来源。
# =============================================================================
...
class ActionDefinition(PermissionModel):
    ...
    flow_step_ids: tuple[str, ...] = Field(min_length=1, max_length=64)
    ...
    @field_validator("flow_step_ids")
    @classmethod
    def normalize_flow_refs(cls, values: tuple[str, ...]) -> tuple[str, ...]:
        if any(not re.fullmatch(_ID_PATTERN, value) for value in values):
            raise ValueError("flow_step_ids must be bounded IDs")
        return _ordered_unique(values, "flow_step_ids")
...
class ResourceDefinitionV2(PermissionModel):
...
class PermissionRuleV2(PermissionModel):
    ...
    required_observers: tuple[str, ...] = Field(min_length=1, max_length=16)
    ...
    @field_validator("required_observers")
    @classmethod
    def normalize_observers(cls, values: tuple[str, ...]) -> tuple[str, ...]:
        if any(not re.fullmatch(_TEXT_PATTERN, value) for value in values):
            raise ValueError("required_observers must contain bounded observer IDs")
        return _ordered_unique(values, "required_observers")
...
class BatchPermissionRuleV2(PermissionModel):
    rule_id: str = Field(pattern=_ID_PATTERN)
    subject_id: str = Field(pattern=_ID_PATTERN)
    action_id: str = Field(pattern=_ID_PATTERN)
    ...
    @field_validator("required_observers")
    @classmethod
    def normalize_batch_observers(cls, values: tuple[str, ...]) -> tuple[str, ...]:
        if any(not re.fullmatch(_TEXT_PATTERN, value) for value in values):
            raise ValueError("required_observers must contain bounded observer IDs")
        return _ordered_unique(values, "required_observers")
    ...
    @model_validator(mode="after")
    def require_bulk_dimension(self) -> BatchPermissionRuleV2:
...
class PermissionContractV2(PermissionModel):
    ...
    resources: tuple[ResourceDefinitionV2, ...] = Field(min_length=1, max_length=4096)
    relations: tuple[RelationFact, ...] = Field(default=(), max_length=8192)
    rules: tuple[PermissionRuleV2, ...] = Field(min_length=1, max_length=8192)
    batch_rules: tuple[BatchPermissionRuleV2, ...] = Field(default=(), max_length=2048)
    ...
    @model_validator(mode="after")
    def validate_references(self) -> PermissionContractV2:
        ...
        for rule in self.rules:
            ...
            if "http" not in rule.required_observers:
                raise ValueError("every permission rule requires the http observer")
            ...
            needs_owner_api = (
                rule.expectation is PermissionExpectation.DENY
                or actions[rule.action_id].side_effect
                or resource.owner_subject_id is not None
                or resource.parent_resource_id is not None
                or bool(rule.context.workflow_states)
                or any(
                    ...
                    for relation_id in rule.relation_path
                )
            )
            if needs_owner_api and "owner_api" not in rule.required_observers:
                raise ValueError("permission rule requires the owner_api observer")
        for batch_rule in self.batch_rules:
            ...
            if "http" not in batch_rule.required_observers:
                raise ValueError("every batch rule requires the http observer")
            if "owner_api" not in batch_rule.required_observers:
                raise ValueError("batch rule requires the owner_api observer")
    ...
    def _validate_relation_path(
        rule: PermissionRuleV2,
        relations: dict[str, RelationFact],
    ) -> None:
        PermissionContractV2._validate_relation_path_values(
    ...
    def validate_flow_references(self, flow_step_ids: tuple[str, ...]) -> PermissionContractV2:
        known = set(flow_step_ids)
        if len(known) != len(flow_step_ids):
            raise ValueError("flow_step_ids must be unique")
        if any(step_id not in known for action in self.actions for step_id in action.flow_step_ids):
            raise ValueError("action references an undeclared Flow step")
        return self
...
def compile_permission_plan(
    ...
    *,
    engine_version: str,
    seed: int = 0,
    flow_step_ids: tuple[str, ...] | None = None,
) -> NormalizedPermissionPlan:
    if flow_step_ids is not None:
        contract.validate_flow_references(flow_step_ids)
    ...
    for rule in contract.rules:
        ...
        cases.append(
            NormalizedPermissionCase(
                ...
                required_observers=rule.required_observers,
                seed=seed,
                engine_version=engine_version,
            )
        )
...
def adapt_v1_ownership_plan(
    ...
    """将 V1 所有权计划投影为 V2 中间表示，不改变原 V1 计划或执行路径。"""
    ...
    for case in plan.cases:
        if (
            case.identity_id not in identity_ids
            or case.resource_id not in resource_ids
            or case.step_id not in step_ids
            or case.rule_id not in rules
        ):
            raise ValueError("V1 ownership plan contains an undeclared reference")
        ...
        cases.append(
            NormalizedPermissionCase(
                ...
                source_rule_id=case.rule_id,
                subject_id=case.identity_id,
                action_id=case.step_id,
                resource_id=case.resource_id,
                relation_path=(f"v1-{case.mutation.value}",),
                context=PermissionContext(),
                expected=PermissionExpectation.DENY,
                required_observers=tuple(sorted(rule.required_observers)),
                seed=plan.seed,
                engine_version=plan.engine_version,
                source_case_id=case.case_id,
                source_step_id=case.step_id,
                source_identity_id=case.identity_id,
                source_resource_id=case.resource_id,
                source_owner_identity_id=case.owner_identity_id,
                source_mutation=case.mutation.value,
            )
        )
    return NormalizedPermissionPlan(
        plan_id=f"v1-plan-{canonical_sha256(tuple(cases))[:32]}",
        source_contract_id=contract.id,
        seed=plan.seed,
        engine_version=plan.engine_version,
        cases=tuple(cases),
