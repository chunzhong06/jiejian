...
class RetryPolicyV1(WorkerControlModel):
    ...
    def validate_bounds(self) -> RetryPolicyV1:
...
class SubmitJobV1(WorkerControlModel):
...
class ClaimJobV1(WorkerControlModel):
...
class RenewLeaseV1(WorkerControlModel):
    ...
    @model_validator(mode="after")
    def validate_new_deadline(self) -> RenewLeaseV1:
...
class RequestCancellationV1(WorkerControlModel):
    job_id: str = Field(pattern=JOB_ID_PATTERN)
    now_us: int = Field(ge=0)


class FencedJobMutationV1(WorkerControlModel):
    job_id: str = Field(pattern=JOB_ID_PATTERN)
    lease_owner: str = Field(pattern=r"^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$")
    fencing_token: int = Field(ge=1)
    now_us: int = Field(ge=0)
...
class CompleteCancellationV1(FencedJobMutationV1):
    pass


class RetryableFailureV1(FencedJobMutationV1):
    reason_code: RetryableFailureCode


class FatalFailureV1(FencedJobMutationV1):
    reason_code: FatalFailureCode


class RecoveryScanV1(WorkerControlModel):
    now_us: int = Field(ge=0)
    limit: int = Field(default=100, ge=1, le=MAX_RECOVERY_SCAN_ITEMS)


class ConfirmRecoveryV1(FencedJobMutationV1):
    proof_type: RecoveryProofType
    operator: RecoveryOperator
    reason_code: RecoveryReasonCode

    @model_validator(mode="after")
    def validate_proof_reason(self) -> ConfirmRecoveryV1:
        expected = {
...
class JobSubmissionResultV1(WorkerControlModel):
    created: bool
    job: JobRecord
    run: RunRecord
