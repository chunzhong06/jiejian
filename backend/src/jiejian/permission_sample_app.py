# =============================================================================
# 阶段 6.2 权限关系 + 6.4 异步因果本机样例
#
# 定位
#   为 Verification V2 覆盖测试提供 fixed/vulnerable/inconclusive loopback 目标。
#
# 安全边界
#   仅绑定 127.0.0.1；令牌只在内存中作不透明比较；reset 需要 loopback 和测试头。
#   这是本机样例进程；正式入口只从环境读取不透明凭据，仍不接入公网。
# =============================================================================
...
class PermissionSampleServer(ThreadingHTTPServer):
    variant: Variant
    tokens: dict[str, str]
    ...
    def __init__(
        ...
        *,
        ...
        if address[0] != "127.0.0.1":
            raise ValueError("permission sample only binds to 127.0.0.1")
        ...
        super().__init__(address, PermissionSampleRequestHandler)
...
class PermissionSampleRequestHandler(BaseHTTPRequestHandler):
    server: PermissionSampleServer
...
def create_permission_sample_server(
    *,
    variant: Variant,
    port: int = 0,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="界鉴阶段 6.2/6.4 权限本机样例")
