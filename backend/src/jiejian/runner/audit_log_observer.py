...
# 顶层的固定 JSONL 轮转族，并把显式 case tag 的有界摘要交回 Observer V2。
# =============================================================================
...
def _file_names(locator: StructuredAuditLogLocatorV2, root: Path) -> list[Path]:
...
def _run_child(invocation: AuditLogObserverInvocationV2, *, utc_now_us: Callable[[], int]) -> ObservationEnvelopeV2:
    started_at_us = utc_now_us()
    locator = invocation.spec.target.locator
    if invocation.spec.observer_type is not ObserverType.STRUCTURED_AUDIT_LOG or not isinstance(locator, StructuredAuditLogLocatorV2):
    ...
    cursor_paths: dict[str, AuditLogStartCursorV2] = {}
    reasons: set[str] = set()
...
def child_main(input_path: str, output_path: str) -> int:
    try:
        ...
        if input_file.stat().st_size > OBSERVER_JSON_MAX_BYTES:
            return 3
        invocation = parse_observer_json(input_file.read_bytes(), AuditLogObserverInvocationV2)
...
def _validate_output_binding(invocation: AuditLogObserverInvocationV2, envelope: ObservationEnvelopeV2) -> None:
...
def run_audit_log_observer(
    ...
    *,
    ...
    invocation = AuditLogObserverInvocationV2(spec=spec, correlation=correlation, phase=phase, start_cursors=start_cursors)
    ...
    try:
        ...
        command = [python_executable or sys.executable, "-B", "-m", "jiejian.runner.audit_observer_process", "--input", str(input_path), "--output", str(output_path)]
        try:
        ...
        try:
            envelope = parse_observer_json(payload, ObservationEnvelopeV2)
