...
def _secret_name(spec: ObserverSpecV2) -> str:
    ...
    if not isinstance(locator, AsyncTaskApiLocatorV2):
...
def _failure_envelope(
    invocation: AsyncTaskObserverInvocationV2,
...
def _build_scope(locator: AsyncTaskApiLocatorV2, *, budget_bytes: int, timeout_us: int) -> TargetGuard:
    ...
    return TargetGuard(scope)


def _request_url(invocation: AsyncTaskObserverInvocationV2) -> str:
    locator = invocation.spec.target.locator
    assert isinstance(locator, AsyncTaskApiLocatorV2)
...
def _run_child(invocation: AsyncTaskObserverInvocationV2, *, utc_now_us: Callable[[], int]) -> ObservationEnvelopeV2:
...
def child_main(input_path: str, output_path: str) -> int:
    try:
        ...
        if input_file.stat().st_size > OBSERVER_JSON_MAX_BYTES:
            return 3
        invocation = parse_observer_json(input_file.read_bytes(), AsyncTaskObserverInvocationV2)
...
def _execution_error(spec: ObserverSpecV2) -> ObserverOutcomeV2:
    return ObserverOutcomeV2(observer_id=spec.observer_id, required=spec.required, status=ObserverOutcomeStatus.EXECUTION_ERROR, reason_codes=(ASYNC_TASK_PROCESS_ERROR,))


def _validate_output_binding(invocation: AsyncTaskObserverInvocationV2, envelope: ObservationEnvelopeV2) -> None:
...
def run_async_task_observer(
    ...
    *,
    ...
    invocation = AsyncTaskObserverInvocationV2(spec=spec, correlation=correlation, phase=phase)
    ...
    try:
        ...
        command = [python_executable or sys.executable, "-B", "-m", "jiejian.runner.async_task_observer_process", "--input", str(input_path), "--output", str(output_path)]
        try:
        ...
        if output_path.stat().st_size > OBSERVER_JSON_MAX_BYTES:
            return AsyncTaskObserverResult(None, _execution_error(spec))
        try:
            envelope = parse_observer_json(output_path.read_bytes(), ObservationEnvelopeV2, known_secrets=tuple(value for key, value in environment.items() if key == source_name and value))
