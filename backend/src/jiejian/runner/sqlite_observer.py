...
# 子进程以 SQLite URI mode=ro 及 query_only 读取，并把结果交回严格 V2 envelope。
# =============================================================================
...
def run_sqlite_observer(
    ...
    *,
    ...
    try:
        ...
        command = [python_executable or sys.executable, "-B", "-m", "jiejian.runner.observer_process", "--input", str(input_path), "--output", str(output_path)]
        try:
        ...
        try:
            ...
            envelope = parse_observer_json(payload, ObservationEnvelopeV2, known_secrets=((source,) if source else ()))
