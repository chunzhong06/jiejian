# Runner 隔离域 Azure Blob 只读对象观察器。
#
# 只读取固定 container/prefix 的 list、head 和有界 get；SAS、URL 与原文不进入工件。
...
def _parse_list(body: bytes, locator: AzureBlobObjectLocatorV2, prefix: str) -> tuple[list[_ListedBlob], str | None]:
...
def _object_url(locator: AzureBlobObjectLocatorV2, name: str, sas: str) -> str:
    return f"{locator.service_url}/{locator.container_name}/{quote(name, safe='/')}?{sas}"


def _list_url(locator: AzureBlobObjectLocatorV2, prefix: str, sas: str, marker: str | None) -> str:
...
def _run_child(invocation: ObserverInvocationV2, *, utc_now_us: Callable[[], int]) -> ObservationEnvelopeV2:
    ...
    if invocation.spec.observer_type is not ObserverType.AZURE_BLOB_OBJECT or not isinstance(locator, AzureBlobObjectLocatorV2):
...
def run_azure_blob_observer(
    ...
    *,
    ...
    if spec.observer_type is not ObserverType.AZURE_BLOB_OBJECT or not isinstance(spec.target.locator, AzureBlobObjectLocatorV2):
    ...
    try:
        ...
        if remaining <= 0:
            ...
            return BlobObserverResult(envelope, evaluate_observer_outcome(envelope, required=spec.required))
        command = [python_executable or sys.executable, "-B", "-m", "jiejian.runner.azure_blob_observer_process", "--input", str(input_path), "--output", str(output_path)]
        try:
