...
#
# 定位
#   Contract 建约、审阅、Diff 与 Drift 能力的命令行适配器
#
# 职责
#   解析命令参数｜复用 ContractWorkbench｜输出稳定 JSON 或错误
#
# 调用链
#   Typer → contract commands → ApplicationContext.contract_workbench
# =============================================================================
...
from ...verification.inputs import load_contract
...
def _workbench_scope(
    context: typer.Context, project_path: Path
) -> Iterator[tuple[ApplicationContext, str]]:
    settings = runtime_settings(context)
    application = ApplicationContext(settings.var_dir)
    try:
        record, _ = application.projects.register(project_path, revalidate=True)
        yield application, record.project_id
    finally:
        application.close()


def _workbench_emit(context: typer.Context, project_path: Path, action) -> None:
    try:
        with _workbench_scope(context, project_path) as (application, project_id):
            emit_json(action(application, project_id))
    except JiejianError as exc:
        fail(exc)


def contract_workspace_command(context: typer.Context, project_path: Path) -> None:
    """登记并重校验 project.yaml，输出契约治理工作台快照。"""
...
def contract_derive_command(
    ...
    include_flow: bool = typer.Option(False, "--include-flow"),
    actor: str = typer.Option("local-user", "--actor"),
) -> None:
    """从选定需求和/或已校验 Flow 派生候选。"""
...
def contract_draft_command(
    ...
    _workbench_emit(
        ...
        lambda application, project_id: application.contract_workbench.create_draft(
            project_id, contract_id, candidate_ids=tuple(candidate), actor=actor
        ).model_dump(mode="json"),
    )
...
def contract_transition_command(
    ...
    def transition(application: ApplicationContext, project_id: str) -> dict:
        if action == "submit":
    ...
    _workbench_emit(context, project_path, transition)


def contract_assessment_command(
    context: typer.Context, project_path: Path, contract_id: str, version: int
) -> None:
...
def contract_drift_command(
    ...
    """输出当前需求、候选、Flow 和观察器能力的 DriftReport。"""
