...
#
# 调用链
#   ContractAnalysisService → build_drift_report → DriftReport
# =============================================================================
...
def build_drift_report(
    ...
    *,
    ...
    contract_rules = {rule.id: rule for rule in contract.snapshot.rules}
    for requirement in sorted(requirements, key=lambda item: item.requirement_id):
        ...
        if not candidates or not any(
            _semantic_key(candidate.rule) in {_semantic_key(rule) for rule in contract.snapshot.rules}
    ...
    capability_by_id = {candidate.rule.id: candidate.rule for candidate in capability_candidates}
    for rule_id in sorted(set(contract_rules) & set(capability_by_id)):
        if _semantic_key(contract_rules[rule_id]) != _semantic_key(capability_by_id[rule_id]):
            entries.append(
                DriftEntry(
    ...
    for llm in llm_candidates:
        for other in explicit:
            ...
            if overlap and llm.rule != other.rule:
                entries.append(
                    DriftEntry(
                        ...
                        rule_ids=(llm.rule.id, other.rule.id),
...
def _semantic_key(rule: ContractRule) -> tuple[str, tuple[str, ...], str]:
    return (rule.kind.value, tuple(sorted(rule.required_observers)), rule.severity)
