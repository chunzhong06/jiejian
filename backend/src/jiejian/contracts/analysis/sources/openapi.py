...
#
# 调用链
#   ContractAnalysisService → build_openapi_candidates → CandidateBatch
# =============================================================================
...
from ..canonical import _candidate, _issue, _issue_key, _rule_id, canonical_sha256
    ...
    for path, path_item in sorted(document["paths"].items(), key=lambda item: str(item[0])):
        ...
        for method, operation in sorted(path_item.items(), key=lambda item: str(item[0])):
            ...
            if not isinstance(operation, Mapping):
                ...
                continue
            kind = RuleKind.FOREIGN_READ if method_upper == "GET" else RuleKind.UNAUTHORIZED_SIDE_EFFECT
