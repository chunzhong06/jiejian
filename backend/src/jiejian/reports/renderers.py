...
# 统一报告 V2 纯投影
#
# 所有格式只消费已经校验的 ReportV2，不读取 publication、数据库或扫描结果。
# =============================================================================
...
from .models import ReportFindingV2, ReportV2, canonical_sha256


def render_json(report: ReportV2) -> bytes:
    return json.dumps(report.model_dump(mode="json"), ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")


def render_html(report: ReportV2) -> bytes:
    rows = []
...
def render_sarif(report: ReportV2) -> bytes:
    results = []
...
def render_junit(report: ReportV2) -> bytes:
    cases = []
    for finding in _findings(report):
...
def render_format(report: ReportV2, output_format: str) -> bytes:
