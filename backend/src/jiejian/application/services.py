...
from ..worker import PersistedExecutionRequestV1
...
def project_source_hash(bundle: ProjectBundle) -> str:
    ...
    digest = hashlib.sha256()
    for label, path in (("project", bundle.project_file.resolve()), ("flow", bundle.project.flow_path.resolve())):
...
def build_execution_request(bundle: ProjectBundle, *, flow: Any = None) -> PersistedExecutionRequestV1:
...
class ApplicationContext:
    """API、CLI 和 serve 共享的运行时依赖。"""

    def __init__(self, var_dir: Path) -> None:
        from ..storage import create_session_factory, create_sqlite_engine, default_database_path, upgrade_database
