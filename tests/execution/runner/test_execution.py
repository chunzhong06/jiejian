...
def _run(monkeypatch, tmp_path: Path, outcome: ExecutionOutcome):
    ...
    runner = execution.RunnerExecutor(_input(), environ={"JIEJIAN_TEST_TOKEN": "subject-secret", "OWNER_READ_ONLY": "owner-secret"}, staging=tmp_path / "staging", clock=lambda: 10)
    try:
        return runner.run_case(_input().project_snapshot.plan.cases[0])
    finally:
        runner.http.close()
