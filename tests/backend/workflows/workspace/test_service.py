...
def test_existing_stale_action_binding_becomes_rebind_task(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert current.primary_task.task_kind == "PREPARE_TEST_IDENTITY"
        assert current.primary_task.can_execute is True
...
def test_never_bound_manual_business_items_do_not_force_rebind_task(
    ...
    try:
        ...
        assert workspace.primary_task.task_kind == "PREPARE_TEST_IDENTITY"
        assert workspace.primary_task.can_execute is False
