...
from tests.fixtures.permission_target import create_complex_permission_test_server
...
def _simple_contract_for_budget() -> PermissionContract:
    return PermissionContract(
...
def test_permission_coverage_is_deterministic_and_records_neighborhoods() -> None:
    data = _simple_contract_for_budget().model_dump(mode="python")
    data["subjects"] = (
        *data["subjects"],
