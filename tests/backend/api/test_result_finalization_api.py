...
def test_gate_report_is_explicit_write_and_requires_gate_result_id() -> None:
    class Reports:
        ...
        def generate_gate(self, run_id: str, gate_result_id: str):
            ...
            return SimpleNamespace(model_dump=lambda **_: {"schema_version": "3", "report_type": "GATE", "report_id": "report_" + "b" * 32})
    ...
    app.include_router(build_results_router(context, SimpleNamespace()))
    with TestClient(app) as client:
        response = client.post(f"/api/runs/{RUN_ID}/reports/gate", json={"gate_result_id": "gate_" + "c" * 32})
        extra = client.post(f"/api/runs/{RUN_ID}/reports/gate", json={"gate_result_id": "gate_" + "c" * 32, "run_id": RUN_ID})
