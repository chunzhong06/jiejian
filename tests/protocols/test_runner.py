...
def test_checked_in_runner_schemas_have_no_drift() -> None:
    schema_root = Path(__file__).parents[2] / "product/protocols/schemas/runner"
...
def test_runner_result_root_and_cleanup_completion_contract_are_current() -> None:
    assert RunnerResult.model_fields["schema_version"].default == "4"
...
def test_runner_result_parser_accepts_only_root_version_four_and_closes_windows() -> None:
    current_evidence = evidence()
    ...
    assert parse_runner_result(raw).schema_version == "4"
    old = json.loads(raw)
