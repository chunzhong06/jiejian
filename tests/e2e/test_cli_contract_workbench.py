# 验证端到端安全闭环中的CLI 权限契约工作台。

from __future__ import annotations

import json
from pathlib import Path

import pytest
from typer.testing import CliRunner

from product.backend.cli.app import app
from tests.fixtures.runner import write_web_test_profile

pytestmark = [pytest.mark.e2e, pytest.mark.database]


def _invoke(runner: CliRunner, var_dir: Path, *args: str):
    result = runner.invoke(
        app,
        ["--var-dir", str(var_dir), "system", "advanced", *args],
    )
    assert result.exit_code == 0, result.stdout + result.stderr + (f"\n{result.exception!r}" if result.exception else "")
    return json.loads(result.stdout)


def test_cli_contract_workbench_profile_lifecycle(tmp_path: Path) -> None:
    runner = CliRunner()
    profile, contract = write_web_test_profile(tmp_path / "inputs")
    var_dir = tmp_path / "var"
    contract_v2 = tmp_path / "contract-v2.json"
    contract_payload = json.loads(contract.read_text(encoding="utf-8"))
    contract_payload["version"] = 2
    contract_v2.write_text(json.dumps(contract_payload), encoding="utf-8")
    workspace = _invoke(runner, var_dir, "contract", "workspace", str(profile))
    assert workspace["project"]["project_id"] == "web-test-project"
    requirement = _invoke(
        runner,
        var_dir,
        "contract",
        "requirement-add",
        str(profile),
        "--text",
        "suggestion id=cli-requirement kind=FOREIGN_READ observations=resource_state severity=high",
        "--tag",
        "pii",
        "--actor",
        "cli-test",
    )
    requirement_derived = _invoke(
        runner,
        var_dir,
        "contract",
        "derive",
        str(profile),
        "--requirement",
        requirement["requirement_id"],
        "--actor",
        "cli-test",
    )
    assert requirement_derived["persisted_candidates"]
    draft = _invoke(
        runner,
        var_dir,
        "contract",
        "draft",
        str(profile),
        "web-test-contract",
        "--snapshot",
        str(contract),
        "--actor",
        "cli-test",
    )
    assert draft["status"] == "DRAFT"
    review = _invoke(runner, var_dir, "contract", "transition", str(profile), "web-test-contract", "1", "submit", "--actor", "reviewer")
    assert review["status"] == "REVIEW"
    active = _invoke(runner, var_dir, "contract", "transition", str(profile), "web-test-contract", "1", "activate", "--actor", "approver")
    assert active["status"] == "ACTIVE"
    assessment = _invoke(runner, var_dir, "contract", "assessment", str(profile), "web-test-contract", "1")
    assert assessment["eligible"] is True
    drift = _invoke(runner, var_dir, "contract", "drift", str(profile), "web-test-contract", "1")
    assert drift["contract_id"] == "web-test-contract"
    revised = _invoke(
        runner,
        var_dir,
        "contract",
        "revise",
        str(profile),
        "web-test-contract",
        "--snapshot",
        str(contract_v2),
        "--actor",
        "cli-test",
    )
    assert revised["version"] == 2
    diff = _invoke(
        runner,
        var_dir,
        "contract",
        "diff",
        str(profile),
        "web-test-contract",
        "2",
        "--from-version",
        "1",
    )
    assert diff["from_version"] == 1
    assert diff["to_version"] == 2


def test_cli_contract_workbench_rejects_invalid_action(tmp_path: Path) -> None:
    runner = CliRunner()
    profile, _ = write_web_test_profile(tmp_path / "inputs")
    result = runner.invoke(
        app,
        [
            "--var-dir",
            str(tmp_path / "var"),
            "system",
            "advanced",
            "contract",
            "transition",
            str(profile),
            "missing",
            "1",
            "bogus",
        ],
    )
    assert result.exit_code != 0
    assert json.loads(result.stdout)["error"]["error_code"] == "INPUT_INVALID"
