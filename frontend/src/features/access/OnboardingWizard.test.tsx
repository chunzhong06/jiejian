...
describe('OnboardingWizard', () => {
  ...
  beforeEach(() => {
    localStorage.clear()
    vi.clearAllMocks()
    mockApi.demoStatus.mockResolvedValue({ status: 'stopped', demo_data: true, session_id: null, project_id: null, run_id: null, job_id: null, message: '内置演示尚未启动。' })
  })

  it('首屏只有新手主入口，高级 YAML 不在向导默认路径', () => {
    render(<OnboardingWizard />)
    expect(screen.getByRole('button', { name: '选择应用文件夹' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: '试用内置演示' })).toBeInTheDocument()
  ...
  it('项目名失焦不单独 PATCH，并在保存启动确认时使用同一 revision', async () => {
    ...
    localStorage.setItem('jiejian.onboarding.session', 'onb_session')
    ...
    expect(mockApi.updateSession).toHaveBeenCalledWith('onb_session', 4, expect.objectContaining({ project_name: '新名称', confirmations: expect.objectContaining({ app_started: true }) }))
  })
  ...
  it('演示成功只把后端返回的真实标识交给控制壳', async () => {
    ...
    mockApi.demoStart.mockResolvedValue({ status: 'running', demo_data: true, session_id: 'onb_demo', project_id: 'onboarding_demo', run_id: 'run_demo', job_id: 'job_demo', message: '演示任务已提交。' })
    render(<OnboardingWizard onSubmitted={onSubmitted} />)
    fireEvent.click(screen.getByRole('button', { name: '试用内置演示' }))
  ...
  it('恢复运行中的演示时只复用后端标识继续查看', async () => {
    const onSubmitted = vi.fn()
    mockApi.demoStatus.mockResolvedValue({ status: 'running', demo_data: true, session_id: 'onb_demo', project_id: 'project_demo', run_id: 'run_demo', job_id: 'job_demo', message: '演示任务正在后台处理。' })
    render(<OnboardingWizard onSubmitted={onSubmitted} />)
