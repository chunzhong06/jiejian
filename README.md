...
开发验证集使用官方协作空间和一个 tests-only 多租户记录应用，覆盖五类权限断裂及漏洞、保护有效、证据不足三种状态，共 30 个 Case。每个 Case 都有独立的正常功能对照；期望结论和断点答案只由被测应用授权源码根之外的 private oracle 保存，不进入产品、MCP 或 Agent 输入。
