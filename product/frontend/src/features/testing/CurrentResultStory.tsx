...
export function CurrentResultStory({ story, onError, onNavigate, requestedCaseId }: { story: ResultStory; onError: (error: ApiError) => void; onNavigate?: (path: string) => void; requestedCaseId?: string | null }) {
  ...
  const [evidenceSide, setEvidenceSide] = useState<'left' | 'right'>('right')
  ...
  useEffect(() => { if (detail && wide) evidencePanel.current?.focus() }, [detail, wide])
  ...
  const openEvidence = async (refs: string[], source?: EvidenceExplanation, breakpoint?: CheckBreakpoint, event?: StoryTraceEvent, coverage?: StoryProofCoverage) => {
    ...
    try {
      ...
      if (values.some(value => value.run_id !== story.run_id || value.action_id !== action?.action_id || value.case.case_id !== action?.case_id)) throw new ApiError('ARTIFACT_MANIFEST', '证据与当前检查项不一致。')
  ...
  const evidenceContent = <>
      ...
      {!loading && !failed && detail?.event && <dl className="evidence-answers"><dt>发生了什么</dt><dd>{traceKindLabels[detail.event.kind]}{detail.event.authorization_decision ? `：${detail.event.authorization_decision === 'DENY' ? '拒绝' : '允许'}` : ''}</dd><dt>证据来源</dt><dd>{detail.event.source_component} · {detail.event.source_location}</dd><dt>能证明什么</dt><dd>已发布路径记录了所选节点及其显式父子关系。</dd><dt>不能单独证明什么</dt><dd>节点存在不等于业务结果已形成，也不能单独证明权限安全；业务后果以独立观察为准。</dd></dl>}
  ...
  return <><div className="result-investigation" data-evidence-open={Boolean(detail && wide)}><div className="result-story" aria-label="已发布的权限与证据故事">
    ...
    {action && comparison && actual ? <article>
      <header className="result-case-heading"><div><p className="editorial-eyebrow">本项判断</p><h2>{action.judgement}</h2></div>
  ...
  </div>{detail && wide && <aside className="result-evidence-panel" data-side={evidenceSide} aria-label="已发布证据" tabIndex={-1} ref={evidencePanel} onKeyDown={event => { if (event.key === 'Escape') { event.stopPropagation(); closeEvidence() } }}><div className="evidence-panel-header"><h2>已发布证据</h2><Button type="text" icon={<CloseOutlined />} aria-label="关闭证据并返回事实" onClick={closeEvidence}/></div>{evidenceContent}<div className="evidence-panel-footer"><Button block type="link" icon={<ArrowLeftOutlined />} onClick={closeEvidence}>返回所选事实</Button></div></aside>}</div>
