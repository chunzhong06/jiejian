...
describe('源码对应', () => {
  ...
  it('读取失败保留明确未知，重新核对只发起读取', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: '重新核对源码' }))
    expect(await screen.findByText('当前源码已不同于这份记录')).toBeInTheDocument()
  ...
  it('只使用确切保存的历史提交，区分同提交与不同内容', async () => {
    read.mockResolvedValue({ ...value(), recorded: { ...value().recorded, git_status: 'AVAILABLE', head: 'f'.repeat(40), has_local_changes: true, observed_at_us: 1, observation_id: 'obs_saved', consistency: 'CONSISTENT' } })
    render(<SourceIdentityPanel {...props}/>)
    expect(await screen.findByText('同一提交，源码内容已经不同')).toBeInTheDocument()
    expect(screen.getByText('当时存在未提交修改。')).toBeInTheDocument()
    expect(screen.queryByText('当时的 Git 提交未记录，不能用当前提交补填。')).not.toBeInTheDocument()
  })
})
