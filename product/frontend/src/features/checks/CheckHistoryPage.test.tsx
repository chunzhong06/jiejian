...
describe('CheckHistoryPage', () => {
  it('一次项目级请求展示本次未覆盖而不是已修复', async () => {
    history.mockResolvedValue({ project_id: 'project-demo', comparisons: [{ run_id: 'run-2', previous_run_id: 'run-1', checked_at_us: 2, policy_epoch: 6, policy_fingerprint: 'd'.repeat(64), relevant_intents: [{ intent_id: `pin_${'e'.repeat(32)}`, revision: 2, intent_hash: 'e'.repeat(64) }], changes: [{ finding_id: 'finding-1', title: '普通用户不应修改文档', subject_group: '普通用户账号', action: '修改', resource: '文档', relation: '拥有', status: 'NOT_COVERED', status_label: '本次未覆盖', explanation: '本次没有执行并充分证明这一权限要求，不能显示为已修复。', severity: 'high', evidence_refs: [], current_verdict: null, occurrence_status: 'DISAPPEARED' }] }] })
    ...
    fireEvent.click(screen.getByText('高级：本次检查标识'))
    expect(screen.getByText('d'.repeat(64))).toBeInTheDocument()
    expect(screen.getByText(`pin_${'e'.repeat(32)}@2:${'e'.repeat(64)}`)).toBeInTheDocument()
