...
#
# 定位
#   ExecutionProfile 与 Web Adapter 之间的受控请求、分类和身份配置边界。
#
# 职责
#   限定模板数据语言｜校验 Slot 与响应提取｜定义确定性结果谓词和身份绑定
#
# 边界
...
from .runner import ProtocolModel
...
class ValueSlot(ProtocolModel):
    # Slot 是单模板内唯一允许的动态值引用；跨步骤 DAG 关系由 10.5B 编译器继续校验。
...
class AuthTargetScope(ProtocolModel):
    """身份端点的独立授权范围；不能借用业务目标的 origin 或预算。"""
    ...
    def validate_scope(self) -> AuthTargetScope:
        ...
        if parsed.scheme not in {"http", "https"} or parsed.hostname is None or parsed.username or parsed.password or parsed.path not in {"", "/"} or parsed.query or parsed.fragment:
        ...
        if address.is_link_local or address.is_reserved or address.is_unspecified or address.is_multicast:
        ...
        for raw_origin in self.allowed_origins:
            ...
            if candidate.scheme not in {"http", "https"} or candidate.hostname is None or candidate.username or candidate.password or candidate.path not in {"", "/"} or candidate.query or candidate.fragment:
                raise ValueError("auth allowed_origins must contain normalized HTTP origins")
            try:
                ...
                raise ValueError("auth allowed_origins must use IPv4 literals") from exc
            origins.append(f"{candidate.scheme}://{candidate_host}:{candidate_port}")
        normalized_origins = tuple(dict.fromkeys(origins))
        normalized_hosts = tuple(dict.fromkeys(str(ipaddress.IPv4Address(item)) for item in self.allowed_hosts))
        normalized_ports = tuple(dict.fromkeys(self.allowed_ports))
        origin = f"{parsed.scheme}://{host}:{port}"
        if origin not in normalized_origins or host not in normalized_hosts or port not in normalized_ports:
...
class StaticHeaderCredential(ProtocolModel):
    ...
    def reject_reserved_name(cls, value: str) -> str:
        name = value.lower()
        if name in {"host", "content-length", "transfer-encoding", "connection", "cookie"} or name.startswith("x-jiejian-"):
...
class OAuth2ClientCredentialsIdentityBinding(ProtocolModel):
    kind: Literal[HttpIdentityKind.OAUTH2_CLIENT_CREDENTIALS] = HttpIdentityKind.OAUTH2_CLIENT_CREDENTIALS
...
class OAuth2RefreshTokenIdentityBinding(ProtocolModel):
    kind: Literal[HttpIdentityKind.OAUTH2_REFRESH_TOKEN] = HttpIdentityKind.OAUTH2_REFRESH_TOKEN
    token_path: str = Field(pattern=_PATH)
    client_id_ref: str = Field(pattern=r"^env:[A-Z][A-Z0-9_]{0,127}$")
