...
class ReportVersions(ReportModel):
    ...
    runner_schema_version: Literal["3"]
    evidence_schema_version: Literal["3"]
    observer_schema_version: Literal["2"]
...
class BaseRunReport(ReportModel):
    ...
    def create(cls, **values: Any) -> BaseRunReport:
        ...
        return cls.model_validate_json(json.dumps({**payload, "canonical_sha256": canonical_sha256(payload)}, ensure_ascii=False), strict=True)
...
def report_id_for(report_type: Literal["BASE", "GATE"], semantic_input_sha256: str) -> str:
    ...
    return f"report_{canonical_sha256(payload)[:32]}"
