...
#
# 边界
#   只返回新领域对象，不写存储，也不允许审阅状态被跳过或就地篡改。
#
# 调用链
#   ContractGovernance → governance rules → ContractVersion / PermissionContract
# =============================================================================

from product.backend.core.contracts.models import ContractAuditAction, ContractAuditEntry, ContractProvenance, ContractVersion
...
def revise_contract_version(
    ...
    *,
    ...
    if snapshot.contract_id != active.contract_id or snapshot.version != next_version:
        raise JiejianError(ErrorCode.CONTRACT_REFERENCE_INVALID, "修订契约快照的 ID 或版本不正确")
