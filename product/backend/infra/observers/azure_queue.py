...
from product.protocols.observer import AzureQueuePeekLocator, CausalityStatus, Correlation, ObservationCompleteness, ObservationEnvelope, ObservationPhase, ObservationProvenance, ObservationWindow, OBSERVER_JSON_MAX_BYTES, ObserverInvocation, ObserverOutcomeStatus, ObserverOutcome, ObserverSpec, ObserverType, ProvenanceType, build_normalized_state, canonical_json_bytes, observer_canonical_sha256, evaluate_observer_outcome, parse_observer_json
...
def _read_response(response: Any, locator: AzureQueuePeekLocator, *, known_secrets: tuple[str, ...]) -> tuple[list[dict[str, Any]] | None, str | None, int]:
...
def _request_url(invocation: ObserverInvocation, sas: str) -> str:
    locator = invocation.spec.target.locator
    assert isinstance(locator, AzureQueuePeekLocator)
...
def _run_child(invocation: ObserverInvocation, *, utc_now_us: Callable[[], int]) -> ObservationEnvelope:
    ...
    locator = invocation.spec.target.locator
    if invocation.spec.observer_type is not ObserverType.AZURE_QUEUE_PEEK or not isinstance(locator, AzureQueuePeekLocator):
    ...
    state_hash = observer_canonical_sha256(state_payload)
    if conflict:
...
def run_azure_queue_observer(
    ...
    *,
    ...
    """在独立进程中执行只读 Peek，并返回与当前 correlation 绑定的队列状态。"""

    invocation = ObserverInvocation(spec=spec, correlation=correlation, phase=phase)
    if spec.observer_type is not ObserverType.AZURE_QUEUE_PEEK or not isinstance(spec.target.locator, AzureQueuePeekLocator):
