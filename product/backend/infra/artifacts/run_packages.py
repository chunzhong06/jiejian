...
#
# 调用链
#   RunnerSupervisor / Publication / Results → validators → staging / final run directory
# =============================================================================
...
class TrustedResultReceipt(BaseModel):
...
class PublicationManifest(BaseModel):
    ...
    result_sha256: str = Field(pattern=SHA256_PATTERN)
    files: tuple[StagedArtifact, ...] = Field(min_length=1, max_length=4097)

    @model_validator(mode="after")
    def validate_file_set(self) -> PublicationManifest:
...
) -> tuple[RunnerResult, tuple[StagedArtifact, ...]]:
...
def validate_published_run(
    ...
    *,
    ...
    """重验最终 Run 目录的清单、文件身份与结果绑定，返回只读可信 publication。"""
    ...
    # 最终目录必须精确闭合：清单之外的文件同样会破坏不可变 publication。
...
def _is_terminal_publish_result(result: RunnerResult) -> bool:
    return result.result_type in _TERMINAL_PUBLISH_TYPES
...
def evidence_records_for_publication(
    ...
    *,
    ...
    return _permission_evidence_records_for_publication(
        final_dir,
        result,
...
def _artifact_record(artifact: RunnerArtifact) -> StagedArtifact:
...
def _parse_runner_result(raw: bytes, *, known_secrets: Sequence[str]) -> RunnerResult:
    """使用唯一当前结果 parser 校验 Runner 输出。"""

    if len(raw) > RUNNER_RESULT_MAX_BYTES or raw.startswith(b"\xef\xbb\xbf"):
...
def _permission_evidence_records_for_publication(
    final_dir: Path,
    result: RunnerResult,
    *,
    created_at_us: int,
    ...
    for artifact in result.artifacts:
        ...
        try:
            raw = path.read_bytes()
            evidence = parse_evidence(raw, known_secrets=known_secrets)
        except (OSError, JiejianError):
            raise JiejianError(ErrorCode.ARTIFACT_MANIFEST, "复杂权限证据索引内容无效") from None
        if Path(artifact.path).name != f"{evidence.evidence_id}.json":
            raise JiejianError(ErrorCode.ARTIFACT_MANIFEST, "复杂权限证据文件名与内容不匹配")
        if len(raw) != artifact.byte_count or hashlib.sha256(raw).hexdigest() != artifact.sha256:
            raise JiejianError(ErrorCode.ARTIFACT_MANIFEST, "复杂权限证据工件哈希不匹配")
        ...
        if expected is None or expected != evidence or evidence.run_id != result.run_id:
            raise JiejianError(ErrorCode.ARTIFACT_MANIFEST, "复杂权限证据与结果不匹配")
    ...
    if expected_by_id:
        raise JiejianError(ErrorCode.ARTIFACT_MANIFEST, " 结果中的证据缺少发布工件")
    return tuple(sorted(records, key=lambda item: item.evidence_id))


def read_publication_manifest(path: Path) -> PublicationManifest:
    try:
...
def write_publication_manifest(path: Path, manifest: PublicationManifest) -> None:
