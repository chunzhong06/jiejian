...
from product.protocols.report import ReportDocument, ReportPackageFile, ReportPackageManifest, parse_report_document
...
class ReportStore:
    ...
    def publish(self, report: ReportDocument) -> ReportPackageManifest:
        ...
        # 先在同一父目录完成全部写入，目录替换才具备原子 publication 语义。
    ...
    def read_format(self, run_id: str, report_id: str, output_format: str) -> bytes:
        """从已校验的 canonical Report 重新投影指定格式。"""
    ...
    def list(self, run_id: str) -> list[dict[str, str]]:
        ...
        for entry in sorted(run_dir.iterdir(), key=lambda item: item.name):
            ...
            item = {"schema_version": "3", "report_id": report.report_id, "run_id": run_id, "report_type": report.report_type, "canonical_sha256": report.canonical_sha256}
    ...
    def _same_package(self, final_dir: Path, files: dict[str, bytes], manifest: ReportPackageManifest) -> bool:
        try:
