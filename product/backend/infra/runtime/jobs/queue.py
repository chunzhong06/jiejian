...
#
# 边界
#   只持久化生命周期意图，不启动 Runner，也不把 JobState 当作安全 Verdict。
#
# 调用链
#   Submission / API → JobQueue → StorageUnitOfWork / JobTargetRegistry
# =============================================================================
...
class JobQueue:
    ...
    def submit(
        ...
        *,
        ...
        try:
            # 幂等键检查和新任务写入必须处于同一事务，唯一约束只负责收敛并发竞态。
            with self._new_uow(known_secrets) as work:
                ...
                work.commit()
                return JobSubmissionResult(created=True, job=job, run=run)
