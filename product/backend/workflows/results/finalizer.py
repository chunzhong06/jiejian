...
class ResultFinalizer:
    ...
    def attach_report_builder(self, report_builder) -> None:
        """在完整 ApplicationCore 装配后接入唯一 ReportBuilder。"""
    ...
    def _process_base_report(self, run_id: str, record: RunFinalizationRecord) -> RunFinalizationRecord:
        ...
        if self._reports is None or record.base_report_state is BaseReportFinalizationState.BLOCKED:
            return record
