# 随 wheel 交付的内置演示目标；不是 samples/ 的官方 Target 真源。
...
class DemoTargetServer(ThreadingHTTPServer):
    daemon_threads = True
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(address, DemoTargetRequestHandler)
...
class DemoTargetRequestHandler(BaseHTTPRequestHandler):
    server: DemoTargetServer
    ...
    def _read_resource(self, resource_id: str, *, trusted_observer: bool) -> None:
        if trusted_observer and self.server.variant == "inconclusive":
            self._send(HTTPStatus.SERVICE_UNAVAILABLE, {"error": "observer_unavailable"})
            return
        actor = self._actor()
        if actor is None:
            self._send(HTTPStatus.UNAUTHORIZED, {"error": "unauthorized"})
...
def create_demo_target_server(
    *,
    ...
    selected_tokens = tokens or {
        "owner": os.environ.get("JIEJIAN_DEMO_OWNER_TOKEN", "demo-owner-token"),
        "attacker": os.environ.get("JIEJIAN_DEMO_ATTACKER_TOKEN", "demo-attacker-token"),
        "peer": os.environ.get("JIEJIAN_DEMO_PEER_TOKEN", "demo-peer-token"),
    }
    return DemoTargetServer(
        ("127.0.0.1", port),
        variant=variant,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="界鉴内置本机演示应用")
    parser.add_argument("--variant", choices=("fixed", "vulnerable", "inconclusive"), required=True)
    parser.add_argument("--port", type=int, default=8765)
    arguments = parser.parse_args()
    server = create_demo_target_server(variant=arguments.variant, port=arguments.port)
    try:
