...
description: 为界鉴项目自有源码和测试新增或更新中文注释。修改任一项目自有 Python 文件（包括 tests、fixture、Alembic 运行环境与 migrations/versions）、product/frontend、scripts、启动批处理或可执行 Sample 时使用；按模块职责维护文件头、公共边界契约和内部原因注释，纯注释任务验证结构等价，功能任务验证新行为与注释一致。
---
...
让维护者在代码附近看懂无法从语法直接得出的原因、公共契约和危险边界，同时把模块定位、调用导航和开发入口交给 Docs。所有项目自有 Python 文件至少保留一行职责明确的 `#` 文件头；简单文件通常一至三行即可，高风险安全、事务、Runner、秘密和进程生命周期文件才按真实需要保留较完整边界卡。注释以中文为主，允许保留 Runner、Evidence、Schema、HTTP 等稳定工程术语。

## 先确定任务模式

- **纯注释模式**：授权只修改说明时，不改变可执行行为、公共格式或测试预期；按下文等价性验证证明结构保持一致。
- **功能开发模式**：用户已授权功能、修复或重构时，注释随最终实现更新。验证新行为、边界和注释一致性，不要求功能修改前后的 AST 相同；只有可单独识别的纯注释补丁才做等价性比较。

两种模式都保留中文职责头、秘密与公共 Schema 边界、已提交 migration 历史约束。注释规则不扩展用户授权，也不阻止已经授权的行为变更。

## 工作顺序

1. 先按 `docs/llms.txt` 读取当前任务和模块入口，再阅读待改文件及其直接调用者，确认真实职责、输入输出、副作用、失败和清理路径；不为写注释扩大成全仓阅读。
...
4. 完成代码修改后按需要补齐职责头、公共契约和内部原因说明，并检查注释是否仍与最终实现一致。
5. 按任务模式验证：纯注释证明结构等价，功能开发证明新行为成立并检查注释匹配；不把结构等价门禁套到已经授权的功能修改上。

## 分模块密度

- Core、Protocol、Runner、Worker、事务、publication、安全边界：中等偏上至较高。必须说清靠近实现的不变量、失败语义和资源/事务边界，但不复制模块参考或调用导航。
...
只有高风险安全、协议、事务、Runner/Worker、秘密或进程生命周期文件确实需要在文件入口同时警示多项局部边界时，才使用完整卡片：
...
Python 文件卡使用 `#`，不得用模块级字符串充当视觉文件头。简单业务文件、Router、CLI、Adapter、Repository、测试或前端文件只用一至三行紧凑说明；不要强行套长卡片。文件在哪里、由谁调用、上下游有哪些模块，优先由模块参考和自动代码参考回答，不在源码头重复维护。
...
注释 Skill 不承担 Markdown 写作规范，也不要求每次局部代码修改同时人工更新多篇 Docs。按变化性质选择唯一同步位置：

```text
只改局部实现
→ 必要时更新附近注释；自动代码参考由 docs -Update 处理

改变模块职责
→ 更新 docs/llms.txt 路由的模块参考
...
生成参考负责公开符号、签名、import 和协议结构等“代码现在是什么”；人工注释负责离实现最近的“为什么必须这样”。两者不得互相复制。
...
- 纯注释任务不改变功能、Schema、数据库、协议、用户界面、测试期望或可执行语句；功能任务只修改已授权边界。
- 不修改第三方代码、纯数据文件或可由构建重新生成的产物；纳入 Git 的 Alembic migration version 是本规则的明确例外，但不得仅为统一排版重写其生成语句。
- 不扩大到未触及文件；纯注释任务发现真实代码问题时只报告，不借注释任务修复。功能任务按其已授权范围处理。
