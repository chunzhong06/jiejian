...
用户先选择已确认 action 和已准备 TestIdentity，独立 Recording Process 恢复会话并打开 headed Chromium。页面准备与登录检查阶段不采集；只有 `capture.ready → start → started` 完整成立后，用户执行的真实业务按钮才进入 `RecordingEvent`。停止后正式结果被 ApplicationCore 消费，才形成可审阅 FlowDraft。

FlowDraft 仍只是候选业务流程。用户确认唯一 TARGET、必要 SETUP、变量和资源位置后才编译为 Flow；Observer、Recovery、ALLOW/DENY 与安全效果在后续安全准备中形成，不由录制器自动猜测。
