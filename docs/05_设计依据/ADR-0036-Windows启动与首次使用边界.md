...
readiness 关键路径创建 ApplicationCore、MCP/control 和CHECK/RECORDING Worker 生命周期，不在控制面执行目标请求；System、`/ready` 与 MCP 共享真实 Worker 状态，CHECK 能力与实际存活状态分别投影。只处理缓存根直接临时项、过期 temp/test 顶层项和有界日志保留的 startup maintenance 位于 ready 后的受控后台任务。该任务由应用生命周期持有，关闭时明确等待，失败只进入现有启动诊断且不撤销 ready。完整 pnpm store、uv cache 等预算统计和 prune 只在用户查看状态或显式维护时执行。

GUI 退出请求复用由根页面取得的当前 control session，并由统一门禁验证真实 `Host` 与精确同源 `Origin` 后进入 Uvicorn/FastAPI shutdown；不再使用可重放的静态控制头。当前链关闭 MCP/control、CHECK/RECORDING Supervisor、受控 Runner/浏览器、身份准备 Manager 与 ApplicationCore；沿 owner token、系统锁和进程树规则安全退出，清理失败保留控制器与引擎；遗留 PID 文件不能阻塞启动或决定锁是否有效。
...
CLI 只公开 `serve`、版本和系统诊断/维护，不保存第二套产品进度；Business Boundary 的创建与批准只在 Web GUI 完成。当前 CHECK 与 Recording 经独立 Worker 和正式 API 装配。官方环境只提供普通输入，结论沿完整 CheckPlan、Worker/隔离 Runner 和发布读取链形成，不能根据变体或 truth 特判；CLI 不另设结果或检查命令。
