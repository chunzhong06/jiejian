...
CLI 只公开 `serve`、版本和系统诊断/维护，不保存第二套产品进度；Business Boundary 的创建与批准只在 Web GUI 完成。完整 Execution、Recording、Result 与官方 Sample 三版本链当前未接回，保留实现不能通过 CLI 菜单或普通项目页面冒充可用。未来 Sample 恢复时仍须沿普通 Contract、Job、真实 Worker/Runner 与已发布结果链运行，产品不得根据变体或 truth 特判结论。
