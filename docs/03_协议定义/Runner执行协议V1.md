...
本文是 [ADR-0002](../02_技术决策/ADR-0002-阶段2执行协议.md) 的字段级协议。实现真源是 `backend/src/jiejian/protocols/runner_v1.py`，稳定公共导入面是 `jiejian.protocols`，签入 Schema 是 `schemas/runner/runner-input-v1.schema.json` 和 `runner-result-v1.schema.json`。
