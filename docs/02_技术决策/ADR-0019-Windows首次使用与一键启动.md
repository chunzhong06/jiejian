...
前端 pnpm 的 `storeDir` 由 `frontend/pnpm-workspace.yaml` 固定为项目根父目录下的 `.pnpm-store`；启动脚本将该配置纳入 Node 依赖指纹，并校验 `node_modules/.modules.yaml` 中的 pnpm 主版本 store 路径。旧路径、缺失或无法解析时仅重建前端 Node 依赖并重新构建，保留现有启动阶段和失败语义。
