...
浏览器 `BrowserContext` 按 identity 独立创建；原始 storage state、trace、Cookie 和明文秘密不得落盘。浏览器传输在发出前检查 TargetScope，并使用有界响应读取；越界、超预算和不支持的长连接进入安全停止。最终 Flow 工件与录制事件均先脱敏、校验和原子发布，数据库完成态不得引用未发布工件。阶段 3 尚未包含公开 API/GUI、动态变量推导增强或更高阶报告格式。
