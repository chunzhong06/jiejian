...
> 状态：当前设计真源；适用阶段：0～6、7.1～7.2、8.1～8.2 与首次使用 C1～C4 已实现，阶段 9 尚未实现；生效日期：2026-08-18
...
- 阶段 8.1 已实现独立 ArtifactCheckJobHandler 和隔离进程：只读取显式 staging/publication 工件与规范 manifest，拒绝路径逃逸、链接、特殊文件、无界遍历、工件执行和在线规则更新，并受文件、字节、时间、压缩层和结果预算限制；Artifact Evidence/Finding 使用独立来源类型和规则集版本，预算/完整性失败只能为 INCONCLUSIVE 或错误。
...
- 阶段 8.2 已实现独立 V2 Report 模型与 Schema、运行时/产物/生命周期/Observer/GateResult 事实收集、四种纯投影、独立原子报告 publication/reader，以及 API、CLI 和报告页的只读接线；报告失败不修改既有 Run publication、Finding、Evidence、Verdict 或 GateResult。
...
| 2026-08-18 | 完成阶段 8.1 受控产物检查协议、固定本地规则集、manifest 约束、Worker 隔离进程、Artifact Evidence/Finding 和最小脱敏/预算/发布闭环；8.2 报告真源仍未实现。 |
| 2026-08-18 | 完成阶段 8.2 统一报告 V2 真源、四格式确定性投影、独立报告 publication/reader、API/CLI/GUI 只读接线；阶段 9 尚未开始。 |
