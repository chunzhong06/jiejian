...
> 状态：当前设计真源；适用阶段：0～6 与首次使用 C1～C4 已实现，7～9 尚未实现；生效日期：2026-08-18
...
- GatePolicy 读取已发布结果和基线生成独立 GateResult，不修改 Verification Verdict。新 BLOCK、重现、覆盖退化、INCONCLUSIVE、required observer 缺失和执行/发布错误默认不能通过；例外必须显式、可追溯且有到期时间。
